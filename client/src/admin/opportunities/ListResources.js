...
import { FiExternalLink, FiSearch, FiLoader, FiAlertCircle, FiEdit2, FiTrash2, FiX, FiCheck, FiPlus } from 'react-icons/fi';
...
const ListResources = () => {
  ...
  const fetchResources = async () => {
    try {
      const response = await apiCall('/api/admin/resources/jobs');
  ...
  const handleUpdate = async (id) => {
    try {
      const response = await apiCall(`/api/admin/resources/jobs/${id}`, {
        method: 'PUT',
        headers: {
