...
const EditJobAlert = ({ alert, onSuccess, onCancel }) => {
  ...
  // Load alert data when the component mounts or when alert prop changes
  useEffect(() => {
    if (alert && alert.id) {
      setName(alert.name || '');
      ...
      // Parse filters
      try {
        const parsedFilters = typeof alert.filters === 'string' 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Call API to update job alert
      const response = await fetch(`/api/seeker/job-alerts/${alert.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to update job alert');
      }
      ...
      setError(error.message || 'Failed to update job alert');
    } finally {
      setLoading(false);
    }
  };
